...
async function trainWithOfficialOptimizer(trainItems, { enableShortTerm = true } = {}) {
  ...
  try {
    ...
    throw new OptimizerDependencyError(
      ...
      '(2) nền tảng deploy (Vercel Node runtime — thường linux-x64-gnu) có bản native/WASM binary ' +
      'tương ứng không (package dùng optionalDependencies theo platform + fallback WASI — nếu deploy ' +
      'trên Vercel, đây thường là do Node File Trace (@vercel/nft) không tự dò được các require() ' +
      'động theo platform của package này và loại nó khỏi function bundle — cần khai báo ' +
      '"includeFiles" trong vercel.json cho node_modules/@open-spaced-repetition/**). ' +
      `Môi trường hiện tại: node=${process.version} platform=${process.platform} arch=${process.arch}. ` +
      `Lỗi gốc: ${e.message}`
    );
  }
...
async function getOptimizerStatus(userId) {
  ...
  try {
    ...
    // Diagnostic: cho FE biết NGAY optimizer chính thức (native binding) có load được trên server
    // hay không — TRƯỚC khi user bấm Run (tránh chờ rồi mới thấy lỗi). Chỉ đọc trạng thái require(),
    // KHÔNG train/không đụng gì tới weights/scheduler.
    const bindingVersion = getOptimizerBindingVersion();
    const base = { report, readiness, bindingAvailable: bindingVersion !== null, bindingVersion };
    if (!r.rows.length) {
