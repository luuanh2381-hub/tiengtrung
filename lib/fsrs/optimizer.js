...
function classifyOptimizerError(e) {
  ...
  if (e instanceof OptimizerDependencyError) {
    ...
    return 'RETRYABLE'; // vd "chạy lỗi khi train ... timeout cấu hình=..." — có thể do tải hệ thống/ngân sách, lần sau có cơ hội xong
  }
...
// chừng (đó là nguồn gốc response non-JSON). V85-HEARTBEAT-FIX — ĐÃ XÁC MINH THÊM (mục 4 yêu cầu sửa
// lần này): trang npm chính thức của package (npmjs.com/package/@open-spaced-repetition/binding) có
// ví dụ dùng đúng chữ ký `computeParameters(items, { enableShortTerm, numRelearningSteps, timeout: 500,
// progress })` — xác nhận `timeout` LÀ option CÓ THẬT, đúng tên, đúng vị trí object thứ 2 đang dùng ở
// đây. Đơn vị chính xác (ms hay giây) vẫn KHÔNG được nêu bằng chữ trong tài liệu — vẫn đoán ms theo
// convention JS phổ biến (Date.now()/setTimeout/fetch timeout đều ms), CỦNG CỐ thêm bởi việc ví dụ
// chính thức dùng cùng bậc độ lớn (500) như giá trị mặc định đã chọn ở đây (45_000) nếu cùng đơn vị.
// V89 — Đặt AN TOÀN dưới ngưỡng THẬT của Vercel Hobby (free) plan: 60 GIÂY CỨNG, bất kể
// vercel.json:functions."api/index.js".maxDuration khai bao nhiêu (300s) — Hobby KHÔNG cho override,
// tự động clamp xuống 60s (đã xác minh qua tài liệu Vercel + nhiều nguồn độc lập 2026: "The 60-second
// function timeout... There is no workaround on the Hobby plan"). ĐÂY LÀ NGUYÊN NHÂN GỐC thật sự của
// bug "mất heartbeat" — không phải giả thuyết, mà là số liệu đã tra cứu — xem
// AUDIT-REPORT-V89-CONTINUATION-CLAIM-BUG.md. Chỉ có ý nghĩa NẾU nguyên nhân là "chạy lâu"; KHÔNG giúp
// được nếu nguyên nhân là native panic/crash không phụ thuộc thời gian.
//
// Nếu deployment THẬT đang ở Pro/Enterprise (ngưỡng thật cao hơn 60s), CHỈNH qua biến môi trường
// FSRS_OPTIMIZER_TIMEOUT_MS (+ FSRS_OPTIMIZER_WORKER_BUDGET_MS/FSRS_OPTIMIZER_MIN_TRAIN_BUDGET_MS bên
// dưới) — KHÔNG sửa số mặc định trong code (mặc định phải an toàn cho trường hợp PHỔ BIẾN NHẤT/rủi ro
// nhất: Hobby).
const OPTIMIZER_COMPUTE_TIMEOUT_MS = Number(process.env.FSRS_OPTIMIZER_TIMEOUT_MS) || 35_000;
...
// OPTIMIZER_WORKER_BUDGET_MS/OPTIMIZER_COMPUTE_TIMEOUT_MS đã được hạ xuống (50s/35s) để nằm AN TOÀN
// dưới ngưỡng 60s đó (xem khai báo 2 hằng số này bên dưới). Khi 1 lượt
...
//      timeout nội bộ 45s của chính nó), failOrRequeue() PHÂN LOẠI (classifyOptimizerError) rồi hoặc
...
// ════════════════════════════════════════════════════
const OPTIMIZER_WORKER_BUDGET_MS = Number(process.env.FSRS_OPTIMIZER_WORKER_BUDGET_MS) || 50_000; // V89: PHẢI nhỏ hơn ngưỡng THẬT — mặc định 60s cứng của Vercel Hobby (xem ghi chú lớn tại OPTIMIZER_COMPUTE_TIMEOUT_MS) — 50s để lại 10s biên an toàn cho cold-start/overhead nền tảng. CHỈNH qua env nếu deployment thật ở Pro/Enterprise (ngưỡng cao hơn).
const OPTIMIZER_MIN_TRAIN_BUDGET_MS = Number(process.env.FSRS_OPTIMIZER_MIN_TRAIN_BUDGET_MS) || 40_000; // V89: >= OPTIMIZER_COMPUTE_TIMEOUT_MS (35s mặc định) + biên cho evaluate/save (~5s) — KHÔNG bắt đầu train nếu ngân sách còn lại dưới mức này
...
async function runOptimizerJob(jobId, userId) {
  ...
  async function failOrRequeue(e, stageAtFailure) {
    ...
    await finishJob(jobId, {
      ...
      errorPublic: retryable ? GENERIC_STALE_ERROR_MESSAGE : GENERIC_OPTIMIZER_ERROR_MESSAGE,
