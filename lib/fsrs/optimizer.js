...
async function runOptimizer(userId, { desiredRetention } = {}) {
  ...
  // V83-FIX-v4 — log mốc BẮT ĐẦU thật sớm: nếu process bị Vercel platform kill giữa chừng (timeout/
  // crash native — không chạm được finishOptimizerRun/fail() nên KHÔNG có log lỗi bình thường), dòng
  // này vẫn có trong Vercel Function Logs, giúp xác nhận request có thật sự bắt đầu chạy runOptimizer
  // hay không (loại trừ khả năng lỗi xảy ra sớm hơn, ví dụ ở tầng auth/routing).
  console.log(`[fsrs-optimizer] runOptimizer bắt đầu — user=${String(userId).slice(0, 3)}*** retention=${desiredRetention ?? 'default'}`);
  try {
...
async function getOptimizerStatus(userId) {
  ...
  try {
    ...
    const base = {
      ...
      optimizerEngineState, engineStatus, // field mới — chi tiết đầy đủ cho admin (Phần 7/11)
    };
    if (!r.rows.length) {
