...
async function finishJobWithCandidate(jobId, userId, { weights, reviewCount, meta }) {
  ...
  try {
    ...
    try {
      ...
      await client.query(
        `UPDATE fsrs_optimizer_jobs SET
           status = 'completed', stage = 'completed', result_meta = $2::jsonb,
           finished_at = now(), heartbeat_at = now(), updated_at = now()
         WHERE id = $1 AND status = 'running'`,
...
async function runOptimizerJob(jobId, userId) {
  ...
  // Heartbeat ĐỘC LẬP — xem giải thích đầy đủ ở khối comment ngay phía trên hàm này. KHÔNG dùng
  // frontend polling, KHÔNG fake progress — chỉ 1 tick định kỳ chứng minh invocation Node của WORKER
  // này (route /worker, giữ sống qua waitUntil — xem lib/runInBackground.js) vẫn còn sống thật.
  let heartbeatTimer = setInterval(() => {
    updateJobHeartbeat(jobId, {}).catch((e) =>
      console.error(`[fsrs-optimizer] job #${jobId} lỗi ghi heartbeat độc lập (bỏ qua, không ảnh hưởng worker):`, e && e.message)
    );
  ...
  try {
    ...
    // nguyên tử có khoá (xem finishJobWithCandidate) THAY CHO 2 lời gọi rời (saveOptimizerCandidate
    // rồi finishJob) như bản trước — đóng đúng khe hở race condition nêu trong báo cáo lỗi. Trả về
    // false nghĩa là job này đã bị stale-recovery đánh 'failed' (hoặc đã bị 1 job/retry khác thay
    // thế) TRƯỚC KHI train xong — worker NÀY (đã thành zombie) phải dừng lại đây, KHÔNG ghi gì thêm,
    // KHÔNG coi đây là lỗi (không gọi finishJob('failed') đè lên nữa — job đã có kết luận riêng rồi).
    const finalized = await finishJobWithCandidate(jobId, userId, {
      weights: personalWeights, reviewCount: report.validReviews, meta,
    });
    if (!finalized) {
      console.log(`[fsrs-optimizer] job #${jobId} — train xong nhưng job đã bị đánh 'failed'/thay thế TRƯỚC đó (worker cũ/zombie) — KHÔNG ghi candidate, active weights giữ nguyên (Phần 6/7/8).`);
      return;
    }
    ...
    // dù thoát bằng return sớm (NOT_READY/finalized=false) hay catch lỗi ngoài dự kiến ở trên.
    stopHeartbeat();
  }
}
