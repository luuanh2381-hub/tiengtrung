...
// là best-effort, bị giới hạn NGHIÊM NGẶT bởi maxDuration CỦA CHÍNH invocation đó (khai 300s trong
// vercel.json cho api/index.js — áp dụng cho CẢ app vì toàn bộ route đi qua 1 file Express duy nhất
// qua rewrite "/api/(.*)" → "/api/index"; con số 300s này CÓ THỂ bị chính plan Vercel của deployment
// TỰ ĐỘNG HẠ THẤP HƠN nữa tuỳ hạng tài khoản — ĐIỀU NÀY CODE KHÔNG THỂ TỰ XÁC MINH, phải kiểm tra
// trực tiếp trên Vercel Dashboard → Project → Settings → Functions của deployment thật). Khi 1 lượt
...
// ════════════════════════════════════════════════════
const OPTIMIZER_WORKER_BUDGET_MS = Number(process.env.FSRS_OPTIMIZER_WORKER_BUDGET_MS) || 200_000; // PHẢI nhỏ hơn maxDuration THẬT (tự kiểm tra Vercel Dashboard — xem ghi chú trên); mặc định 200s an toàn dưới ngưỡng phổ biến nhất 300s nhưng vẫn CÓ THỂ cần chỉnh nếu plan thấp hơn
const OPTIMIZER_MIN_TRAIN_BUDGET_MS = Number(process.env.FSRS_OPTIMIZER_MIN_TRAIN_BUDGET_MS) || 60_000; // >= OPTIMIZER_COMPUTE_TIMEOUT_MS (45s mặc định) + biên an toàn cho evaluate/save (~15s) — KHÔNG bắt đầu train nếu ngân sách còn lại dưới mức này
