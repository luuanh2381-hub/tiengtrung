...
// ════════════════════════════════════════════════════
const {
  getPool, reviewFsrsCard: dbReviewFsrsCard, previewAutoRating: dbPreviewAutoRating,
  countDueFsrsCards, getDueFsrsCards, getNewWordsByLessonOrder, countNewWordsInLessons,
  getTodayStudyCounts, getWeakFsrsCards, vnDateKey, getLessonsByWordIds,
} = require('../db');
...
// FSRS Personal Optimizer (Phần 11): getUserActiveWeights trả null (dùng default) hoặc mảng 21
// weights riêng nếu user đã Apply — có cache TTL riêng, cùng kiểu với userSettingsCache bên dưới.
// (invalidateActiveWeightsCache dùng trực tiếp từ api/index.js qua fsrsOptimizer, không qua đây.)
const { getUserActiveWeights } = require('./optimizer');
...
async function setUserRetention(userId, desiredRetention) {
  if (!isAllowedRetention(desiredRetention)) {
    throw new PublicError(`desired_retention không hợp lệ — chỉ chấp nhận ${ALLOWED_RETENTIONS.join(', ')}`);
  }
  const client = await getPool().connect();
  try {
    await ensureUserSettingsTable(client);
...
// V101 fix (yêu cầu người dùng — "Học không giới hạn" trước đây bị hiểu/code SAI thành bỏ hẳn 2
// giới hạn dailyReviewLimit/dailyNewLimit, khiến dashboard/phiên học nhảy thẳng lên TOÀN BỘ backlog
// (164/146...) bỏ qua luôn 2 ô số đã cấu hình): đúng ý người dùng, "không giới hạn" CHỈ có nghĩa là
// không bị CHẶN LẠI vì đã học đủ trong ngày (bỏ phần TRỪ ĐI reviewToday/newToday) — 2 con số
// dailyReviewLimit/dailyNewLimit vẫn LUÔN là mức trần cho MỖI LƯỢT lấy phiên, dù bật hay tắt. Nhờ
// vậy user học được nhiều LƯỢT tuỳ ý trong ngày (không bị chặn "hết hạn mức, quay lại ngày mai"),
// nhưng MỖI lượt vẫn đúng số từ đã cấu hình trong Cài đặt hằng ngày — không còn bỏ qua cài đặt.
async function getDailyBudget(userId, ui) {
  const { dailyReviewLimit, dailyNewLimit, newOnlyAfterDue, unlimitedStudy } = studySettings(ui);
  const [{ newToday, reviewToday }, totalDue] = await Promise.all([
  ...
  const remainingReviewSlots = unlimitedStudy ? dailyReviewLimit : Math.max(0, dailyReviewLimit - reviewToday);
  const remainingNewSlots = unlimitedStudy ? dailyNewLimit : Math.max(0, dailyNewLimit - newToday);
  // newOnlyAfterDue là 1 cài đặt riêng, ĐỘC LẬP với unlimitedStudy — "không giới hạn số lần học
  // trong ngày" không có nghĩa là bỏ qua luôn cài đặt "chỉ học từ mới sau khi hết backlog".
  const blockedByBacklog = newOnlyAfterDue && totalDue > remainingReviewSlots;
  return { remainingReviewSlots, remainingNewSlots, blockedByBacklog, totalDue };
}
...
async function getTodayOverview(userId, ui) {
  const safeUi = ui || {};
  const { scopeLessons, allLessonsWithVocab } = await resolveStudyScope(safeUi);
  const currentLesson = resolveCurrentLesson(safeUi, scopeLessons);
  const [dueInScope, newInScope, weakCards, budget] = await Promise.all([
    ...
    getDailyBudget(userId, safeUi),
  ]);
  // V98 fix: trừ đúng theo Cài đặt hằng ngày (giống hệt getStudySession() sẽ thực sự lấy) — số
  // hiển thị ở đây và số thật sự nhận được khi bấm "Bắt đầu học" giờ LUÔN khớp nhau.
  const dueCount = Math.min(dueInScope, budget.remainingReviewSlots);
  const newInCurrentLesson = budget.blockedByBacklog ? 0 : Math.min(newInScope, budget.remainingNewSlots);
  return {
    dueCount,
    newInCurrentLesson,
    currentLesson,
    ...
    // V100 fix ("chưa đúng số từ cài đặt trong Cài đặt hằng ngày"): khi còn backlog due (có thể ở
    // Quyển/bài KHÁC ngoài phạm vi đang chọn) và "Chỉ học từ mới sau khi hết backlog" đang bật,
    // newInCurrentLesson bị ép về 0 dù dailyNewLimit còn slot — tổng số từ nhận được (review+new)
    // vì vậy KHÔNG khớp dailyReviewLimit+dailyNewLimit. Trả thêm 2 field này để FE giải thích rõ lý
    // do ngay trên dashboard thay vì để user thấy số "không khớp" mà không hiểu vì sao (đây là hành
    // vi CHỦ ĐỘNG, không phải bug — xem docs/fsrs.md mục "Daily limit").
    blockedByBacklog: budget.blockedByBacklog,
    totalDue: budget.totalDue,
  };
}
...
async function getStudySession(userId, ui) {
  ...
  const [{ scopeLessons, allLessonsWithVocab }, budget] = await Promise.all([
    resolveStudyScope(safeUi),
    getDailyBudget(userId, safeUi),
  ]);
  const currentLesson = resolveCurrentLesson(safeUi, scopeLessons);
  const { remainingReviewSlots, remainingNewSlots, blockedByBacklog, totalDue } = budget;
  ...
  // V103 (yêu cầu người dùng — hiện "từ này thuộc những bài nào" lúc đang học/ôn): tra 1 LẦN DUY
  // NHẤT cho CẢ phiên (thay vì mỗi thẻ 1 query riêng — session có thể lên tới hàng trăm thẻ).
  const lessonsMap = await getLessonsByWordIds([...dueCards, ...newCards].map(r => r.resolved_word_id));

  const session = [
    ...dueCards.map(c => ({ type: 'review', word: formatFsrsCard(c, lessonsMap) })),
    ...newCards.map(w => ({ type: 'new', word: formatVocabWord(w, lessonsMap) })),
  ];
...
// ── Bổ sung "Highlight System Rating": xem TRƯỚC gợi ý rating hệ thống mà KHÔNG ghi gì lên FSRS —
//     dùng cho FE hiện 4 nút Again/Hard/Good/Easy + highlight nút hệ thống đề xuất trong lúc chờ
//     người dùng xác nhận/đổi (Yêu cầu bổ sung). ──
async function previewRating({ userId, hz, l, answerCorrect, responseTimeMs, answerChanges }) {
  return dbPreviewAutoRating({ userId, hz, l, answerCorrect, responseTimeMs, answerChanges });
}

// ── HÀM DUY NHẤT api layer gọi để ghi 1 lượt review FSRS. ──────────────────────────────────
// ratingOverride (bổ sung "Highlight System Rating"): rating do NGƯỜI DÙNG chọn tay (hoặc do
// countdown ở FE tự chọn khi hết giờ) — nếu hợp lệ, THẮNG gợi ý tự động của hệ thống (xem
// lib/db.js:reviewFsrsCard). Không truyền (undefined/null) thì giữ nguyên hành vi V67 cũ.
async function reviewCard({ userId, hz, l, answerCorrect, responseTimeMs, answerChanges, idempotencyKey, ratingOverride }) {
  // FSRS Personal Optimizer (Phần 11): lấy song song, KHÔNG thêm round-trip nối tiếp nào so với
  // trước đây — getUserActiveWeights có cache TTL riêng (mirror userSettingsCache) nên trong tuyệt
  // đại đa số lượt review (giữa 2 lần cache hết hạn) đây là 1 lần đọc Map trong bộ nhớ, không phải
  // 1 query Postgres mới.
  const [{ desiredRetention }, personalWeights] = await Promise.all([
    getUserSettings(userId),
    getUserActiveWeights(userId),
  ]);
