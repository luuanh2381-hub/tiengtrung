...
function reviewCard(existingCard, ratingStr, now, desiredRetention) {
  ...
  const { scheduler } = getSchedulerForRetention(desiredRetention);
...
function previewSchedule(existingCard, now, desiredRetention) {
  const reviewTime = now || new Date();
  const card = existingCard || emptyCard(reviewTime);
  const { scheduler } = getSchedulerForRetention(desiredRetention);
