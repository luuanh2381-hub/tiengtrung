...
// Bản đầu của bulkUpsertVocab xử lý TUẦN TỰ từng dòng (1 SELECT + 1 INSERT/UPDATE + 1 INSERT-lesson
// riêng cho MỖI dòng) — với vài trăm dòng, hàng trăm round-trip DB nối tiếp nhau dễ vượt giới hạn
// thời gian của serverless function (Vercel), khiến kết nối bị ngắt giữa chừng và trình duyệt báo
// lỗi network ("Failed to fetch") dù dữ liệu KHÔNG hề bị hỏng (transaction tự rollback sạch).
//
// planBulkUpsert() là hàm THUẦN (không đụng DB) tính ra "kế hoạch" cần làm, dựa trên dữ liệu ĐÃ LẤY
// SẴN từ 1 câu SELECT DUY NHẤT (existingByHz) — cho phép lib/db.js thực thi bằng vài câu SQL hàng
// loạt (bulk INSERT/UPDATE) thay vì lặp round-trip theo từng dòng, mà VẪN giữ NGUYÊN 100% quy tắc
// nghiệp vụ cũ (merge field theo overwrite, không tạo trùng, luôn thêm lesson relationship).
//
// existingByHz: Map<hz, {id, hz, py, vi, tag, hanviet}> — bản ghi canonical hiện có cho mỗi hz
//   (nếu 1 hz có nhiều dòng do CHƯA chạy migration dedupe, caller phải tự chọn id nhỏ nhất trước
//   khi đưa vào Map này — xem lib/db.js).
// rows: mảng { hz, py, vi, l, tag } đã validate (hz/vi không rỗng, l là số nguyên >= 1).
// overwrite: boolean.
//
// Trả về:
//   newWords: [{ hz, py, vi, tag, l, lessons: [sorted] }]  — cần INSERT vocab_words mới
//   updates:  [{ id, py, vi, tag }]                         — cần UPDATE metadata (CHỈ khi thực sự đổi)
//   existingLessonNeeds: [{ wordId, lessons: [sorted] }]    — quan hệ lesson cần đảm bảo cho từ ĐÃ CÓ
//   counts: { added, updated }                              — biết chắc ngay, không phụ thuộc bước SQL sau
function planBulkUpsert(existingByHz, rows, overwrite) {
  const groups = groupBy(rows, r => r.hz);
  const newWords = [];
  const updates = [];
  const existingLessonNeeds = [];
  let added = 0, updated = 0;

  for (const [hz, groupRows] of groups) {
    const existing = existingByHz.get(hz);
    const lessons = [...new Set(groupRows.map(r => r.l))].sort((a, b) => a - b);

    if (!existing) {
      // Chưa từng tồn tại — gộp tuần tự các dòng TRÙNG hz NGAY TRONG batch này thành 1 record duy
      // nhất (vd Excel liệt kê cùng 1 từ ở 2 bài khác nhau) — luôn coi là "overwrite" giữa các dòng
      // mới với nhau vì đều ngang hàng, cần gộp đủ thông tin, không có khái niệm "giữ bản cũ".
      let acc = { hz, py: groupRows[0].py, vi: groupRows[0].vi, tag: groupRows[0].tag, hanviet: null };
      for (let i = 1; i < groupRows.length; i++) {
        const merged = mergeVocabRecords(acc, groupRows[i], true);
        acc = { hz, py: merged.py, vi: merged.vi, tag: merged.tag, hanviet: null };
      }
      newWords.push({ hz, py: acc.py, vi: acc.vi, tag: acc.tag, l: lessons[0], lessons });
      added++;
    } else {
      // Đã tồn tại — merge tuần tự TỪNG dòng trong group vào state hiện tại (đúng thứ tự xuất hiện
      // trong file, giữ hành vi y hệt bản xử lý tuần tự cũ).
      let acc = { hz: existing.hz, py: existing.py, vi: existing.vi, tag: existing.tag, hanviet: existing.hanviet };
      let anyChanged = false;
      for (const r of groupRows) {
        const merged = mergeVocabRecords(acc, r, !!overwrite);
        if (merged.changed) anyChanged = true;
        acc = { hz: acc.hz, py: merged.py, vi: merged.vi, tag: merged.tag, hanviet: merged.hanviet };
      }
      if (anyChanged) { updates.push({ id: existing.id, py: acc.py, vi: acc.vi, tag: acc.tag }); updated++; }
      existingLessonNeeds.push({ wordId: existing.id, lessons });
    }
  }
  return { newWords, updates, existingLessonNeeds, counts: { added, updated } };
}

module.exports = {
  mergeFuller, mergeSticky, splitMeaningParts, mergeMeaning,
  mergeVocabRecords, pickCanonical, mergeManyVocabRecords,
  pickSurvivingFsrsCard, groupBy, planBulkUpsert,
};
