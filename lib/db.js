...
async function getVocabByLessons(lessons) {
  ...
  try {
    ...
    const r = await client.query('SELECT hz, py, vi, l, tag FROM vocab_words WHERE l = ANY($1::int[]) ORDER BY l, id', [lessons]);
    return r.rows;
  } finally {
    client.release();
  }
...
async function getAllVocabWords() {
  ...
  try {
    await ensureVocabTable(client);
    const r = await client.query('SELECT hz, py, vi, l, tag FROM vocab_words ORDER BY l, id');
    return r.rows;
  } finally {
    client.release();
  }
