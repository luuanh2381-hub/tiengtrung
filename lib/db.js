...
async function reviewFsrsCard({ userId, hz, l, answerCorrect, responseTimeMs, answerChanges, desiredRetention, idempotencyKey }) {
  const client = await getPool().connect();
  try {
    await ensureFsrsTables(client);
    await client.query('BEGIN');
    ...
    if (idempotencyKey) {
      ...
      if (dup.rows.length) {
        ...
        return {
          ok: true,
          card: cardRow.rows.length ? { ...rowToCard(cardRow.rows[0]), hz, l } : null,
          wasNew: false,
          autoRating: h.rating,
          duplicate: true,
