...
// ── V93 (audit "thống nhất từ vựng giữa các bài", Phần 6/7/8/9) — VIẾT LẠI TOÀN BỘ so với bản cũ.
//
// TRƯỚC ĐÂY: 1 câu INSERT ... ON CONFLICT (hz, l) hàng loạt — coi (hz, l) là identity, nên cùng 1
// chữ Hán ở 2 bài khác nhau LUÔN tạo ra 2 dòng vocab_words riêng biệt (đúng gốc rễ của vấn đề "từ
// vựng giữa các bài không được thống nhất").
//
// GIỜ: identity là hz (đã có unique index thật vocab_words_hz_idx SAU KHI
// scripts/migrate-vocab-identity.js chạy xong — trước đó vẫn AN TOÀN nhờ resolveCanonicalWordId
// luôn lấy id NHỎ NHẤT một cách nhất quán dù lỡ còn sót duplicate). Với MỖI từ trong danh sách
// nhập: tìm bản ghi hiện có theo hz; có rồi thì merge metadata theo `overwrite` (lib/vocab-merge.js
// — KHÔNG BAO GIỜ ghi đè bằng giá trị rỗng) rồi ĐẢM BẢO có quan hệ (word_id, lesson) trong
// vocab_lessons — quan hệ này LUÔN được thêm dù overwrite bật hay tắt (Phần 9: "Nếu overwrite=OFF:
// existing vocabulary + NEW LESSON RELATIONSHIP + giữ metadata cũ"); chưa có thì INSERT mới.
//
// Xử lý TUẦN TỰ từng dòng (không gộp thành 1 câu SQL hàng loạt như bản cũ) vì cần đọc-rồi-quyết-
// định cho từng từ — đánh đổi lấy tốc độ chậm hơn 1 chút để đổi lấy ĐÚNG-ĐẮN dữ liệu (đây là thao
// tác ADMIN, chạy không thường xuyên, không nằm trên đường link của người học). Dùng 1 cache Map
// trong bộ nhớ theo hz để 2 dòng CÙNG hz nhưng khác lesson NGAY TRONG CÙNG 1 lần import (vd file
// Excel liệt kê "学习" ở cả bài 1 lẫn bài 8) nhận ra nhau mà không cần round-trip DB thừa.
async function bulkUpsertVocab(client, words, overwrite) {
  let added = 0, updated = 0, invalid = 0, skipped = 0, lessonLinked = 0;
  const localCache = new Map(); // hz -> { id, hz, py, vi, tag, hanviet }
  for (const raw of words) {
    ...
    const py = w.py != null ? String(w.py).trim() : '';
    const tag = w.tag || null;

    let existing = localCache.get(hz);
    if (existing === undefined) {
      const sel = await client.query(
        'SELECT id, hz, py, vi, tag, hanviet FROM vocab_words WHERE hz = $1 ORDER BY id ASC LIMIT 1',
        [hz]
      );
      existing = sel.rows.length ? sel.rows[0] : null;
    }

    if (!existing) {
      // Từ HOÀN TOÀN MỚI — INSERT vocab_words + link lesson (Phần 8: 电脑 chưa tồn tại -> INSERT).
      const ins = await client.query(
        `INSERT INTO vocab_words (hz, py, vi, l, tag) VALUES ($1,$2,$3,$4,$5)
      ...
      added++;
      localCache.set(hz, row);
      continue;
    }

    // Từ ĐÃ TỒN TẠI (Phần 6/8/9) — KHÔNG BAO GIỜ tạo vocab_words mới cho hz này nữa.
    const merged = vocabMerge.mergeVocabRecords(existing, { hz, py, vi, tag }, !!overwrite);
    if (merged.changed) {
      await client.query(
        `UPDATE vocab_words SET py = $1, vi = $2, tag = $3 WHERE id = $4`,
        [merged.py, merged.vi, merged.tag, existing.id]
      );
      updated++;
    }
    const link = await client.query(
      `INSERT INTO vocab_lessons (word_id, lesson) VALUES ($1,$2)
       ON CONFLICT (word_id, lesson) DO NOTHING RETURNING word_id`,
      [existing.id, l]
    );
    if (link.rows.length) lessonLinked++;
    else if (!merged.changed) skipped++; // thật sự không có gì mới: từ đã có, đã ở đúng bài này rồi
    localCache.set(hz, { id: existing.id, hz, py: merged.py, vi: merged.vi, tag: merged.tag, hanviet: merged.hanviet });
  }
