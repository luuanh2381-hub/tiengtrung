...
// ── GHI 1 LƯỢT REVIEW — giao dịch có khoá dòng (FOR UPDATE) để chống double-click / nhiều tab
//     ghi đè sai lịch FSRS của cùng 1 thẻ (Phần 27). Đây là nơi DUY NHẤT ghi/đổi 1 fsrs_card. ──
async function reviewFsrsCard({ userId, hz, l, ratingStr, answerCorrect }) {
  const client = await getPool().connect();
  try {
    await ensureFsrsTables(client);
    await client.query('BEGIN');
    ...
    const beforeCard = before.reps === 0 && before.state === 0 && !before.last_review ? null : rowToCard(before);
    const now = new Date();
    ...
    return { ok: true, card: { ...row, hz, l }, wasNew };
  } catch (e) {
    await client.query('ROLLBACK').catch(() => {});
    throw e;
  } finally {
