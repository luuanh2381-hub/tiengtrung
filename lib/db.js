...
//     mỗi lần thêm bài mới"): admin tự thêm/sửa/xoá 1 "Quyển"/giáo trình (tên hiển thị + khoảng số
//     bài liên tục) ngay trên web. Các Quyển/HSK CÓ SẴN của app (hardcoded ở BOOKS_RANGES trong
//     lib/fsrs/studyScope.js + BOOKS trong js/ui.js) KHÔNG di chuyển vào đây — bảng này CHỈ chứa
//     các Quyển admin tự thêm MỚI, được GỘP thêm vào lúc chạy (xem getEffectiveBookRanges ở
//     studyScope.js và loadBookDefs ở js/ui.js) — không tốn công migrate dữ liệu cũ, không rủi ro
//     đổi hành vi các Quyển đang hoạt động tốt. ──────────────────────────────────────────────────
let bookDefsTableReady = null;
async function ensureBookDefsTable(client) {
  if (bookDefsTableReady) return bookDefsTableReady;
  bookDefsTableReady = (async () => {
    try {
      await client.query(`
        CREATE TABLE IF NOT EXISTS book_defs (
          id SERIAL PRIMARY KEY,
          name TEXT NOT NULL,
          group_name TEXT NOT NULL DEFAULT '🆕 Giáo trình tự thêm',
          lesson_from INT NOT NULL,
          lesson_to INT NOT NULL,
          sort_order INT NOT NULL DEFAULT 0,
          created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
          CHECK (lesson_from >= 1 AND lesson_to >= lesson_from)
        )
      `);
    } catch (e) {
      bookDefsTableReady = null; // cho phép thử lại ở lần gọi sau nếu lần này lỗi (mất kết nối DB...)
      throw e;
    }
  })();
  return bookDefsTableReady;
}

let cachedBookDefs = null;
let cachedBookDefsAt = 0;
const BOOK_DEFS_CACHE_TTL_MS = 60000;

// Đọc TOÀN BỘ Quyển do admin tự khai báo (KHÔNG gồm các Quyển hardcoded có sẵn). Cache ngắn giống
// getVocabCounts vì hàm này nằm trên đường "mỗi request tính phạm vi học" (resolveStudyScope) —
// gọi rất thường xuyên, không nên round-trip DB mỗi lần.
async function getBookDefs() {
  if (cachedBookDefs && (Date.now() - cachedBookDefsAt) < BOOK_DEFS_CACHE_TTL_MS) return cachedBookDefs;
  const client = await getPool().connect();
  try {
    await ensureBookDefsTable(client);
    const r = await client.query('SELECT * FROM book_defs ORDER BY sort_order, id');
    cachedBookDefs = r.rows;
    cachedBookDefsAt = Date.now();
    return cachedBookDefs;
  } finally {
    client.release();
  }
}
function invalidateBookDefsCache() { cachedBookDefs = null; }

function validateBookDefInput({ name, groupName, lessonFrom, lessonTo }) {
  const cleanName = String(name || '').trim();
  const cleanGroup = String(groupName || '').trim() || '🆕 Giáo trình tự thêm';
  const from = parseInt(lessonFrom, 10);
  const to = parseInt(lessonTo, 10);
  if (!cleanName) throw new PublicError('Vui lòng nhập tên hiển thị cho Quyển/giáo trình.');
  if (cleanName.length > 60) throw new PublicError('Tên hiển thị quá dài (tối đa 60 ký tự).');
  if (!Number.isFinite(from) || !Number.isFinite(to) || from < 1 || to < from) {
    throw new PublicError('Khoảng số bài không hợp lệ — "Từ bài" phải ≥ 1 và ≤ "Đến bài".');
  }
  return { name: cleanName, groupName: cleanGroup, from, to };
}

async function createBookDef({ name, groupName, lessonFrom, lessonTo }) {
  const clean = validateBookDefInput({ name, groupName, lessonFrom, lessonTo });
  const client = await getPool().connect();
  try {
    await ensureBookDefsTable(client);
    // Chỉ kiểm tra trùng khoảng bài với 1 Quyển TỰ THÊM khác đã có — kiểm tra trùng với Quyển
    // hardcoded (BOOKS_RANGES) nằm ở api/index.js (nơi có sẵn cả 2 danh sách, tránh vòng lặp
    // require ngược lại đây vì studyScope.js đã require từ lib/db.js).
    const overlap = await client.query(
      `SELECT id, name FROM book_defs WHERE lesson_from <= $2 AND lesson_to >= $1 LIMIT 1`,
      [clean.from, clean.to]
    );
    if (overlap.rows.length) {
      throw new PublicError(`Khoảng bài ${clean.from}-${clean.to} trùng với "${overlap.rows[0].name}" đã khai báo — hãy chọn khoảng bài khác.`);
    }
    const r = await client.query(
      `INSERT INTO book_defs (name, group_name, lesson_from, lesson_to) VALUES ($1, $2, $3, $4) RETURNING *`,
      [clean.name, clean.groupName, clean.from, clean.to]
    );
    invalidateBookDefsCache();
    return r.rows[0];
  } finally {
    client.release();
  }
...
async function updateBookDef(id, { name, groupName, lessonFrom, lessonTo }) {
  const clean = validateBookDefInput({ name, groupName, lessonFrom, lessonTo });
  const client = await getPool().connect();
  try {
    await ensureBookDefsTable(client);
    const overlap = await client.query(
      `SELECT id, name FROM book_defs WHERE id != $3 AND lesson_from <= $2 AND lesson_to >= $1 LIMIT 1`,
      [clean.from, clean.to, id]
    );
    if (overlap.rows.length) {
      throw new PublicError(`Khoảng bài ${clean.from}-${clean.to} trùng với "${overlap.rows[0].name}" đã khai báo — hãy chọn khoảng bài khác.`);
    }
    const r = await client.query(
      `UPDATE book_defs SET name = $1, group_name = $2, lesson_from = $3, lesson_to = $4 WHERE id = $5 RETURNING *`,
      [clean.name, clean.groupName, clean.from, clean.to, id]
    );
    if (r.rows.length === 0) throw new PublicError('Không tìm thấy Quyển/giáo trình này (có thể đã bị xoá).');
    invalidateBookDefsCache();
    return r.rows[0];
  } finally {
    client.release();
...
async function deleteBookDef(id) {
  const client = await getPool().connect();
  try {
    await ensureBookDefsTable(client);
    // CHỈ xoá dòng khai báo (tên + khoảng bài) — KHÔNG đụng vocab_words/vocab_lessons: đây thuần
    // là nhãn hiển thị, xoá đi thì các bài đó rơi vào "🆕 Từ vựng mới (tự nhận diện)" ở màn chọn
    // bài (xem mergeDiscoveredLessonsIntoBooks ở js/ui.js) thay vì mất dữ liệu từ vựng.
    const r = await client.query('DELETE FROM book_defs WHERE id = $1 RETURNING id', [id]);
    invalidateBookDefsCache();
...
async function clearVocab() {
  ...
  try {
    ...
    await client.query('BEGIN');
    const countRes = await client.query('SELECT COUNT(*)::int AS c FROM vocab_words');
    // V97 fix ("Lỗi server nội bộ" khi xoá): gỡ hết quan hệ từ-bài trước — luôn an toàn, không bảng
    // nào tham chiếu vocab_lessons.
    await client.query('DELETE FROM vocab_lessons');
    // Từ V93, fsrs_cards.word_id/review_history.word_id tham chiếu vocab_words(id) nhưng KHÔNG có
    // ON DELETE CASCADE — trước đây hàm này DELETE FROM vocab_words không điều kiện, nên hễ có 1 từ
    // đã từng được user ôn tập là Postgres báo lỗi vi phạm khoá ngoại (23503), cả API trả về 500
    // ("Lỗi server nội bộ") dù ROLLBACK sạch (không mất dữ liệu, nhưng xoá được 0 từ). Nay CHỈ xoá
    // hẳn những từ KHÔNG có fsrs_cards/review_history nào tham chiếu; từ đã có lịch sử ôn tập được
    // GIỮ LẠI dạng "mồ côi" (đã gỡ khỏi mọi bài ở trên nên không còn hiện ở đâu trong app) để không
    // phá vỡ FSRS/lịch sử ôn tập cũ của user.
    await client.query(
      `DELETE FROM vocab_words v
         WHERE NOT EXISTS (SELECT 1 FROM fsrs_cards fc WHERE fc.word_id = v.id)
           AND NOT EXISTS (SELECT 1 FROM review_history rh WHERE rh.word_id = v.id)`
    );
    await client.query('COMMIT');
    cachedVocabCounts = null; // V72
    return countRes.rows[0].c; // giữ nguyên ý nghĩa cũ: tổng số từ đã gỡ khỏi mọi bài học
  } catch (e) {
    await client.query('ROLLBACK').catch(() => {});
    throw e;
  } finally {
...
async function deleteVocabLesson(l) {
  ...
  try {
    ...
    if (wordIds.length) {
      ...
      // V97 fix ("Lỗi server nội bộ" khi xoá bài): Phần 16 chỉ tính dependency qua vocab_lessons —
      // nhưng từ V93, fsrs_cards.word_id/review_history.word_id CŨNG tham chiếu vocab_words(id)
      // (KHÔNG CASCADE). Xoá bài chứa 1 từ đã được ôn tập trước đây → Postgres vi phạm khoá ngoại
      // (23503) → lỗi này KHÔNG phải PublicError → lộ ra ngoài thành "Lỗi server nội bộ" chung
      // chung, ROLLBACK sạch nhưng admin không xoá được cả bài (dù không mất dữ liệu). Nay tính
      // thêm 2 dependency đó: từ có lịch sử ôn tập được GIỮ LẠI dạng "mồ côi" (đã gỡ khỏi bài này ở
      // trên nên không còn hiện trong bài nào) thay vì làm cả request thất bại.
      await client.query(
        `DELETE FROM vocab_words v WHERE v.id = ANY($1::int[])
           AND NOT EXISTS (SELECT 1 FROM vocab_lessons vl WHERE vl.word_id = v.id)
...
async function removeVocabWordFromLesson(id, lesson) {
  ...
  try {
    ...
    if (remain.rows.length === 0) {
      // V97 fix (xem deleteVocabLesson): chỉ xoá hẳn vocab_words nếu KHÔNG có fsrs_cards/
      // review_history nào tham chiếu (thêm ở V93, không CASCADE) — nếu không sẽ vi phạm khoá ngoại
      // và lộ "Lỗi server nội bộ" ra ngoài. `fullyDeleted` vẫn = true vì ý nghĩa hiển thị cho admin
      // là "không còn thuộc bài nào" — đúng, bất kể hàng vocab_words có bị xoá vật lý hay giữ lại
      // dạng mồ côi vì lý do FSRS.
      await client.query(
        `DELETE FROM vocab_words WHERE id = $1
           AND NOT EXISTS (SELECT 1 FROM fsrs_cards WHERE word_id = $1)
           AND NOT EXISTS (SELECT 1 FROM review_history WHERE word_id = $1)`,
        [id]
      );
      fullyDeleted = true;
    }
...
// V103 (yêu cầu người dùng — hiện thêm "từ này thuộc những bài nào" lúc ĐANG HỌC/ÔN, không chỉ ở
// màn admin): tra CÙNG LÚC danh sách bài của NHIỀU từ (theo word_id) trong 1 câu query — dùng ở
// getStudySession()/`/api/study/weak-words` để gắn field `lessons` vào TỪNG thẻ của cả phiên học,
// tránh N+1 query (mỗi phiên có thể tới hàng trăm thẻ — xem UNLIMITED_SLOT).
async function getLessonsByWordIds(wordIds) {
  const ids = (wordIds || []).filter(id => Number.isFinite(id));
  if (ids.length === 0) return {};
  const client = await getPool().connect();
  try {
    await ensureVocabTable(client);
    const r = await client.query(
      'SELECT word_id, array_agg(lesson ORDER BY lesson) AS lessons FROM vocab_lessons WHERE word_id = ANY($1::int[]) GROUP BY word_id',
      [ids]
    );
    const map = {};
    for (const row of r.rows) map[row.word_id] = row.lessons;
    return map;
  } finally {
    client.release();
  }
...
async function deleteVocabWord(id) {
  ...
  try {
    await ensureVocabTable(client);
    await client.query('BEGIN');
    const existing = await client.query('SELECT id FROM vocab_words WHERE id = $1', [id]);
    if (existing.rows.length === 0) {
      await client.query('COMMIT');
      return false;
    }
    // V97 fix (xem deleteVocabLesson): gỡ quan hệ lesson trước (luôn an toàn), rồi CHỈ xoá hẳn
    // vocab_words nếu không có fsrs_cards/review_history nào tham chiếu (FK thêm ở V93, không
    // CASCADE) — trước đây DELETE thẳng không điều kiện nên xoá 1 từ đã từng được ôn tập sẽ vi
    // phạm khoá ngoại và lộ ra ngoài thành "Lỗi server nội bộ" chung chung.
    await client.query('DELETE FROM vocab_lessons WHERE word_id = $1', [id]);
    await client.query(
      `DELETE FROM vocab_words WHERE id = $1
         AND NOT EXISTS (SELECT 1 FROM fsrs_cards WHERE word_id = $1)
         AND NOT EXISTS (SELECT 1 FROM review_history WHERE word_id = $1)`,
    ...
    await client.query('COMMIT');
    cachedVocabCounts = null; // V72
    return true; // đã xử lý xong (gỡ khỏi mọi bài, xoá hẳn hoặc giữ mồ côi tuỳ có lịch sử ôn tập)
  } catch (e) {
    await client.query('ROLLBACK').catch(() => {});
    throw e;
  } finally {
...
async function getDueFsrsCards(userId, limit, scopeLessons) {
  ...
  try {
    ...
    const scopeClause = hasScope
      ...
      : '';
    const r = await client.query(
      `SELECT f.*, v.py, v.vi, v.tag, v.hanviet, v.id AS resolved_word_id
       FROM fsrs_cards f
...
async function getNewWordsByLessonOrder(userId, lessonOrder, limit) {
  ...
  try {
    await ensureFsrsTables(client);
    const r = await client.query(
      `SELECT hz, py, vi, l, tag, hanviet, vid AS resolved_word_id FROM (
...
async function getWeakFsrsCards(userId, limit) {
  ...
  try {
    await ensureFsrsTables(client);
    const r = await client.query(
      `SELECT f.*, v.py, v.vi, v.tag, v.hanviet, v.id AS resolved_word_id, COALESCE(rh.wrong_count, 0)::int AS wrong_count
       FROM fsrs_cards f
       JOIN vocab_words v ON (f.word_id IS NOT NULL AND v.id = f.word_id)
                          OR (f.word_id IS NULL AND v.hz = f.hz AND v.l = f.l)
       LEFT JOIN LATERAL (
...
module.exports = {
  ...
  getBookDefs, createBookDef, updateBookDef, deleteBookDef, // V102 (tự khai báo giáo trình trên web)
  getLessonsByWordIds, // V103 (hiện "thuộc những bài nào" lúc đang học/ôn)
