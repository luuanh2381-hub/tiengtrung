...
async function reviewFsrsCard({ userId, hz, l, answerCorrect, responseTimeMs, answerChanges, desiredRetention }) {
  const client = await getPool().connect();
  try {
    await ensureFsrsTables(client);
    await client.query('BEGIN');
    ...
    await client.query(
      `INSERT INTO review_history
         (user_id, hz, l, rating, answer_correct, reviewed_at,
          ...
          response_time_ms, answer_changes, auto_rating, elapsed_days)
...
module.exports = {
  ...
  countKnownFsrsWords, getKnownCountsForUsers,
